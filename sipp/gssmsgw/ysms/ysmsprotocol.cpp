...
YSMSProtocol::YSMSProtocol(const ServiceIdentifier& id, 
						   const string & configFile, 
						   HandlerInterface * handler)
:BaseProtocol(id, configFile, handler)
,m_logger(Logger::getInstance("msggw"))
,m_msgLogger(Logger::getInstance("YSMS"))
,m_cacheList(8192, 60)
,m_cycleTimes(0)
{
	m_server = NULL;
...
bool	YSMSProtocol::open()
{
	...
	if(NULL == m_server)
	{
		m_server = new YSMSServer( m_myID.m_appref, this );
...
int	YSMSProtocol::sendMessage(const ServiceIdentifier& source, UserTransferMessagePtr utm)
{ 
...
void	YSMSProtocol::handleClientEvt(const ServiceIdentifier& receiver, const UserTransferMessagePtr& utm)
{
	switch(utm->getMessageId())
	{
...
void	YSMSProtocol::handleClientMsg(cacti::Stream& content)
{
	YSMSHeader header(0);
	content.reset();
	header.fromStream(content);
	YSMSPacketPtr packet = YSMSPacketFactory::createPacket(header.command);
	...
	case YSMS_Login_Resp:
		{
			YSMSLoginResp* pkg = static_cast<YSMSLoginResp*>(packet.get());
			m_logger.info("[%02d] login resp %d\n", m_myID.m_appref, pkg->status);

			if(pkg->status == YSMS::Status_OK)
				setStatus(ST_READY);
			else
				setStatus(ST_ERROR);
		}
	...
	case YSMS_Deliver:
		{
			YSMSDeliver* pkg = static_cast<YSMSDeliver*>(packet.get());
			handleDeliver(pkg);
		}
		break;
	...
	case YSMS_GW_REPORT:
		{
			YSMSGWReport* pkg = static_cast<YSMSGWReport*>(packet.get());
			handleGwReport(pkg);
		}
		break;
	...
	case YSMS_TERM_REPORT:
		{
			YSMSTermReport* pkg = static_cast<YSMSTermReport*>(packet.get());
...
bool	YSMSProtocol::sendLogin()
{
	...
	//bool ret = m_server->sendPackage(packet);
...
bool	YSMSProtocol::sendActiveTest()
{
	//YSMSActiveTest packet;
	//packet.status = 0;
...
bool	YSMSProtocol::sendActiveTestResp()
{
	//YSMSActiveTestResp packet;
	//packet.status = 0;
...
int	YSMSProtocol::sendSubmit(const ServiceIdentifier& source, UserTransferMessagePtr utm)
{
	YSMSSubmit packet;
...
bool	YSMSProtocol::handleGwReport(YSMSGWReport* pkg)
{
#pragma message("to do ...")
...
bool	YSMSProtocol::handleTermReport(YSMSTermReport* pkg)
{
#pragma message("to do ...")
...
bool	YSMSProtocol::handleDeliver(YSMSDeliver* pkg)
{
	string LineNo;
	LineNo.assign(pkg->LineNo.begin(), pkg->LineNo.end());
	StringUtil::trim(LineNo);
...
int	YSMSProtocol::respMessage(const ServiceIdentifier& source, UserTransferMessagePtr utm, u32 ret)
{
	...
	return YSMS::Status_OK;
}

void YSMSProtocol::test(int unit)
{
	if(unit == 1)
	{
