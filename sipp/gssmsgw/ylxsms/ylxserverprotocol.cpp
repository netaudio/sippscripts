...
YLXServerProtocol::YLXServerProtocol(const ServiceIdentifier& id,
...
YLXServerProtocol::~YLXServerProtocol()
{
	close();
	if(m_server)
	{
...
bool	YLXServerProtocol::open()
{
	...
	if(NULL == m_server)
	{
		m_server = new YLXServer( m_myID.m_appref,this );
...
#pragma message("to do ...")
	...
	setStatus(ST_INIT);
	m_logger.info("[%02d] ylx server (0x%x) (%s, %d) init\n",
		m_myID.m_appref, 
		m_version,
	...
	m_logger.info("[%02d] ylx server start  %s\n", m_myID.m_appref, ret == true ? "ok" : "failed");
...
bool YLXServerProtocol::sendPackage(YLXPacket& pkg)
{
	Stream stream;
...
bool YLXServerProtocol::sendPackage(char *buf,int len)
{

	bool ret = m_server->sendbuf(buf,len);
	if(ret)
	{
		m_statPackets.totalSendPackets ++;
	}
...
bool YLXServerProtocol::sendPackage(YLXPacketCachePtr pkg)
{
...
bool	YLXServerProtocol::activeTest()
{
