...
namespace SGIP
{
	...
	enum _CommandCode
	{
		SGIP_Login	                        = 0x00000001,  //客户端登录请求
		SGIP_Login_Resp                     = 0x80000001,  //客户端登录应答
		SGIP_Submit                         = 0x00000003,  //提交短消息
		SGIP_Submit_Resp                    = 0x80000003,  //提交短消息应答
		SGIP_Deliver                        = 0x00000004,  //下发短消息
		SGIP_Deliver_Resp                   = 0x80000004,  //下发短消息应答

		SGIP_Report                        = 0x00000005,  //下发短消息
		SGIP_Report_Resp                   = 0x80000005,  //下发短消息应答

		SGIP_Active_Test                    = 0x00000008,  //连接激活测试
		SGIP_Active_Test_Resp               = 0x80000008,  //连接激活测试回应
		SGIP_Exit	                        = 0x00000002,  //退出请求
		SGIP_Exit_Resp	                    = 0x80000002,  //退出应答
		SGIP_Query	                        = 0x00000007,  //SP统计查询请求
		SGIP_Query_Resp                     = 0x80000007,  //SP统计查询应答
	};
	...
	enum _LoginMode 
	{
		SP_SMG	= 1, //*1：SP向SMG建立的连接，用于发送命令
		SMG_SP		=2, //*2：SMG向SP建立的连接，用于发送命令
		SMG_SMG=3, //*3：SMG之间建立的连接，用于转发命令
		SMG_GNS=4, //*4：SMG向GNS建立的连接，用于路由表的检索和维护
		GNS_SMG=5,//*5：GNS向SMG建立的连接，用于路由表的更新
		GNS_GNS=6,//*6：主备GNS之间建立的连接，用于主备路由表的一致性
		SP_SMG_TEST=11,//*11：SP与SMG以及SMG之间建立的测试连接，用于跟踪测试
	};
