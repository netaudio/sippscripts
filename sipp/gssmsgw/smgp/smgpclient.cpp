...
SMGPClient::SMGPClient(int clientID, ServerInterface* owner)
...
bool SMGPClient::sendPackage(SMGPPacket& pkg)
{
	Stream stream;
...
bool SMGPClient::sendStream(cacti::Stream& stream)
{
	return send(stream.getWriteBuffer(), (int)stream.size()) == stream.size();
}

void SMGPClient::onConnected()
{
...
int  SMGPClient::onReceive()
{
	...
	if(left == 0 || left >= SMGP::Max_Packet_Size)
	{
...
int SMGPClient::recvError(int ret)
{			
	if(ret == 0)
		return RECV_CLOSE;
	else if(ret > 0)
...
bool SMGPClient::enqueue(Stream& stream)
{
	//assert(m_owner);
