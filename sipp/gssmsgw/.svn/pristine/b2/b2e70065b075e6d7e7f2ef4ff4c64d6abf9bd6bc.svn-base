#ifndef  __SMGP_CACHE_CONTROL
#define  __SMGP_CACHE_CONTROL
...
public:
	SMGPPacketCache();
	//SMGPPacketCache(SMGPSubmit& pkg);
	~SMGPPacketCache();
	u32		packetID;
	...
	Timestamp   addStamp;
	Stream content;
	//SMGPPacketCache& operator=(SMGPPacketCache& other);
};
...
{
public:
	SMGPCacheControl(SMGPProtocol* owner,int  max_sendwindow_size,int recvtimeout);
	~SMGPCacheControl();
	bool open();
	int	 getCanPutMsgSize(u8 priority);
	void response(u32 packetid);
	bool put(SMGPSubmit& pkg);
...
private:
	...
	bool getSendPacket(SMGPPacketCachePtr& packet);
...
//	void updateSendPoint();
	...
	vector<SMGPPacketCachePtr> hightCacheQueue;
	vector<SMGPPacketCachePtr> normalCacheQueue;
	vector<SMGPPacketCachePtr> lowCacheQueue;
...
public:
	...
	void						process();
private:
	SMGPCacheControl*			m_owner;
	SMGPClient*					m_client;
	cacti::Thread				m_thread;
	int activetime;//unit :ms
};
class SMGPCacheTimoutThread
{
public:
	SMGPCacheTimoutThread();
	~SMGPCacheTimoutThread();
	bool						init();
	void						start();
	void						process();
