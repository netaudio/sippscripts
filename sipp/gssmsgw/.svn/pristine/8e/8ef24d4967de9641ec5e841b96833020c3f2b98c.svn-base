...
SGIPProtocol::SGIPProtocol(const ServiceIdentifier& id,
...
bool	SGIPProtocol::open()
{
	...
	if(NULL == m_client)
	{
		m_client = new SGIPClient( m_myID.m_appref,this );
...
#pragma message("to do ...")
	...
	setStatus(ST_INIT);
	m_logger.info("[%02d] sgip client (0x%x) (%s, %d) init, support long sms(%d)\n",
		m_myID.m_appref, 
		m_version,
...
//	m_stopSendThread = false;	
	...
	m_logger.info("[%02d] sgip client connect to server %s\n",
...
bool SGIPProtocol::sendPackage(SGIPPacket& pkg)
{
...
bool SGIPProtocol::sendPackage(SGIPPacketCachePtr pkg)
{
